...
/**
 * Servlet implementation class CollegeAdminChangePassword
 */
@WebServlet("/CollegeAdminChangePassword")
public class CollegeAdminChangePassword extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ResultSet resultset = statement.executeQuery("select upass from tbladmin where upass='" + cpassword
					+ "' and email ='" + session.getAttribute("email") + "'");
			if (resultset.next()) {
